...
Template.quote_list.events =
	...
	'change input.select': (e, t) ->
		selected = if Quotes.findOne({ _id: @_id }).selected is 'checked' then '' else 'checked'
		Quotes.update(@_id, { $set: { 'selected': selected } })

Template.quote_list.quotes = Template.quote_carousel.quotes = -> Quotes.find({}).fetch()
...
addStock = (symbol) -> refreshStocks symbol unless Quotes.find({ symbol: symbol }).count() 
	
removeStock = (id) -> Quotes.remove id
	
removeStocks = -> _.each Quotes.find({selected: 'checked'}).fetch(), (stock) -> Quotes.remove stock._id
